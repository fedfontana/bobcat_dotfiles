-- Adapted from https://github.com/dmmulroy/.dotfiles
vim.api.nvim_create_user_command("RotateWindows", function()
	...
	for _, window_number in ipairs(window_numbers) do
		...
		if not vim.tbl_contains(ignored_filetypes, filetype) then
			if eligible_windows == 2 then
				vim.api.nvim_err_writeln("You can only swap 2 open windows.")
				return
			end

			table.insert(windows_to_rotate, { window_number = window_number, buffer_number = buffer_number })
			eligible_windows = eligible_windows + 1
		end
	end

	if eligible_windows == 0 then
		vim.api.nvim_err_writeln("There is no window to rotate.")
		return
	elseif eligible_windows == 1 then
